...
export class UserGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
	...
	async addFriend(@MessageBody() data: FRIEND_REQUEST_DATA) {
		try {
			...
			const user_recv = this.userService.findConnectedUserByUsername(data.client_recv);//this.connectedUser.find((user: any) => {return user.username === data.client_recv})
			const db_user_recv = await this.userRepository.findOne({ where:{username:data.client_recv} })
			const user_emit = this.userService.findConnectedUserByUsername(data.client_emit);//this.connectedUser.find((user: any) => {return user.username === data.client_emit})
